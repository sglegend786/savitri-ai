...
export default function PharmacyPublicPage() {
  ...
  useEffect(() => {
    const fetchPharmacy = async () => {
      try {
        const res = await fetch(`http://localhost:5000/api/pharmacies/slug/${slug}`);
        if (!res.ok) {
  ...
  useEffect(() => {
    if (pharmacy && pharmacy._id) {
      const fetchReviews = async () => {
        ...
        try {
          const res = await fetch(`http://localhost:5000/api/reviews?pharmacyId=${pharmacy._id}`);
          if (res.ok) {
            const data = await res.json();
  ...
  const handleReviewSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // Refresh reviews
        const revRes = await fetch(`http://localhost:5000/api/reviews?pharmacyId=${pharmacy._id}`);
        if (revRes.ok) {
